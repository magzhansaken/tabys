-- =====================================================================
-- 082_journal_values.sql — в журнале должны быть ЗНАЧЕНИЯ, а не только
-- названия полей.
--
-- НАЙДЕНО СВЕРКОЙ. Прочитал журнал как человек, который через полгода
-- разбирает спор. Три записи не дают ответа:
--
--   «Изменены цены платформы» — и всё. Спросят «когда подняли Старт с
--     6 900 до 7 900» — ответа нет: запись есть, что менялось — нет;
--
--   «этап: paid» — кодом. В воронке человек видит столбец «Оплатил»,
--     а в журнале «paid», и связать их не может;
--
--   «правка карточки · город» — какой был и какой стал, неизвестно.
--     Спросят «кто сменил город на Алматы» — запись есть, ответа нет.
--
-- Данные для всего этого В ЖУРНАЛЕ УЖЕ ЛЕЖАЛИ: они пишутся при
-- действии. Их просто не разбирали при показе.
-- =====================================================================
