-- =====================================================================
-- 081_journal_readable.sql — журнал должен читаться через полгода.
--
-- НАЙДЕНО СВЕРКОЙ. Прочитал журнал так, будто разбираю спорную оплату
-- спустя полгода. Три записи читаются плохо:
--
--   «Магжан правка карточки — Береке» — ЧТО поправил? Телефон?
--     Название? Город? Это самая частая запись, и по ней ничего не
--     восстановить;
--
--   «Ерлан заведён магазин — Береке · Береке» — название задвоено:
--     оно приходит и своим столбцом, и из подробностей;
--
--   «Оплата подтверждена — 6900 ₸ · 3 мес. · партнёру 1035 ₸» — нет
--     названия магазина. Деньги есть, а чьи — нет. (Оно приходит
--     отдельным полем и показывается кабинетом — проверено, эта
--     запись читается верно.)
-- =====================================================================
  ...
  SELECT pa.id, pa.seq, pa.at, pa.actor_name, pa.actor_id, pa.action,
    ...
    nullif(concat_ws(' · ',
      ...
      CASE WHEN pa.action = 'card_updated' THEN
        nullif(concat_ws(', ',
          CASE WHEN pa.details ? 'name'        THEN 'название' END,
          CASE WHEN pa.details ? 'city'        THEN 'город' END,
          CASE WHEN pa.details ? 'ownerName'   THEN 'владелец' END,
          CASE WHEN pa.details ? 'ownerPhone'  THEN 'телефон' END,
          CASE WHEN pa.details ? 'dealStage'   THEN 'этап' END,
