-- =====================================================================
-- 075_journal_cursor.sql — листание журнала теряло записи.
--
-- НАЙДЕНО СВЕРКОЙ. Листание шло по ВРЕМЕНИ записи: «покажи то, что
-- раньше такого-то момента». Пока все записи в разные секунды, это
-- работает.
--
-- Но массовое действие пишет несколько записей ОДНИМ мгновением. И
-- тогда: страница кончилась на такой записи, следующая просит «раньше
-- этого момента» — и ПРОПУСКАЕТ все остальные записи той же секунды.
--
-- Проверено: шесть записей одним моментом, листание по две — потеряно
-- пять из двадцати одной. Человек листает журнал и не видит, что
-- часть событий просто исчезла.
--
-- Исправлено: у записи появился порядковый номер, и листаем по нему.
-- Номер уникален всегда, сколько бы записей ни легло разом.
-- =====================================================================
ALTER TABLE platform_audit ADD COLUMN IF NOT EXISTS seq bigserial;
CREATE INDEX IF NOT EXISTS platform_audit_seq_idx ON platform_audit(seq DESC);

DROP FUNCTION IF EXISTS platform_journal(text, uuid, timestamptz, uuid, uuid, text, integer);
