-- =====================================================================
-- 086_login_failed.sql — неудачный вход в журнал.
--
-- НАЙДЕНО ОПИСЬЮ ДЕЙСТВИЙ. Все меняющие действия пишутся в журнал —
-- проверил все тридцать одно. Кроме входа.
--
-- Успешный вход писать и не надо: он бывает по десять раз в день и
-- засорит след. Он уже виден столбцом «Был в системе».
--
-- А вот НЕУДАЧНЫЙ не виден нигде. Кто-то подбирает пароль к панели, где
-- все деньги платформы, все клиенты и все партнёры, — и владелец
-- узнает об этом только когда вход УДАСТСЯ. То есть поздно.
--
-- Неудачная попытка — редкость. Каждая стоит записи, и вес у неё
-- «доступ»: это про то, кто куда попал.
-- =====================================================================
CREATE OR REPLACE FUNCTION platform_journal_title(p_action text)
RETURNS text
IMMUTABLE LANGUAGE sql AS $$
  SELECT CASE p_action
    ...
    ELSE NULL END;
$$;
GRANT EXECUTE ON FUNCTION platform_journal_title(text) TO shop_app;


DROP FUNCTION IF EXISTS platform_journal(text, uuid, bigint, uuid, uuid, text, integer);
