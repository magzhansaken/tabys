-- =====================================================================
-- 071_search_fix.sql — поиск клиентов не работал вовсе.
--
-- НАЙДЕНО СВЕРКОЙ РАЗГРАНИЧЕНИЯ. Проверял, не найдёт ли партнёр чужих
-- через поиск, — и обнаружил, что поиск не находит НИЧЕГО и НИКОГО:
-- любой запрос возвращает весь список.
--
-- ПРИЧИНА. Отбор по телефону написан как
--   phone_tail(owner_phone) = phone_tail(p_q)
--
-- Для запроса «Астана» phone_tail даёт пустую строку. У клиента без
-- телефона владельца — тоже пустая. Пустая равна пустой, условие
-- истинно, и клиент попадает в выдачу. А раз условий несколько через
-- ИЛИ, в выдачу попадают ВСЕ.
--
-- Заметить это на глаз нельзя: список не пустой, строки есть, поиск
-- «работает». Просто он всегда показывает всё.
--
-- ИСПРАВЛЕНО: по телефону ищем, только если в запросе есть цифры.
-- =====================================================================
