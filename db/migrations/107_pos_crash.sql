-- =====================================================================
-- 107_pos_crash.sql — падение кассы в журнале платформы.
--
-- ЧАСТЬ 20 РАЗБОРА ИХ КАССЫ. Их слово точное: КАНАРЕЙКА — птица в
-- шахте падает первой, и все узнают о газе.
--
-- Касса падает у клиента в Шымкенте. Кассир перезапускает её и
-- работает дальше — ему некогда звонить. Мы не узнаём НИКОГДА, а
-- падение повторяется каждый день у десяти клиентов.
--
-- Запись уже приходит, но в журнале она безымянная. Даём название и
-- подробность: что упало и в какой версии — иначе владелец видит
-- «pos_crash» и не понимает, беда это или мелочь.
-- =====================================================================
...
SECURITY DEFINER SET search_path = public LANGUAGE sql STABLE AS $$
  SELECT pa.id, pa.seq, pa.at, pa.actor_name, pa.actor_id, pa.action,
    ...
    -- кабинете по-разному.
    CASE pa.action
      WHEN 'pos_crash'            THEN 'Касса упала'
    ...
    nullif(concat_ws(' · ',
      ...
      -- ЧТО ИМЕННО РЕШИЛИ по заявке. «Заявка одобрена» без итога
      -- ничего не говорит: одобрили что и почём? А это деньги —
      -- через полгода спросят именно об этом.
      CASE WHEN pa.action IN ('request_approved', 'request_rejected')
             AND pa.details ? 'effect'
           THEN pa.details->>'effect' END,

      -- ЧТО ИМЕННО УПАЛО. «Касса упала» без подробности не говорит,
      -- беда это или мелочь: одна и та же ошибка у десяти клиентов —
      -- это наша ошибка, а разные — это их железо.
      CASE WHEN pa.action = 'pos_crash' THEN
        nullif(concat_ws(' · ',
          pa.details->>'message',
          CASE WHEN pa.details ? 'version' THEN 'версия ' || (pa.details->>'version') END), '')
      END,
