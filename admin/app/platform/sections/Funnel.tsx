...
import { api, money, daysWord, type Me } from '../lib';
...
export default function Funnel({ me }: { me: Me }) {
  ...
  const load = async () => {
    try { setData(await api('/funnel')); setErr(''); }
  ...
  const move = async (id: string, stage: string) => {
    try {
      ...
      setMoving(null); setNote(''); await load();
    } catch (e: any) { setErr(e.message); }
  };
      ...
      <div style={{ display: 'grid', gap: 14,
        ...
        {data.stages.map((st: any) => (
          <Card key={st.key} title={`${st.title} · ${st.count}`}>
            <div style={{ fontSize: 13, color: C.dim, marginTop: -4, marginBottom: 4 }}>{st.hint}</div>
            ...
            <div style={{ display: 'grid', gap: 8 }}>
              {st.cards.map((c: any) => (
                <div key={c.id} style={{
                  border: `1px solid ${c.cold ? C.amber : C.line}`,
              ...
              {st.cards.length === 0 && (
                <div style={{ fontSize: 13, color: C.dim }}>пусто</div>
              )}
            </div>
          </Card>
        ))}
