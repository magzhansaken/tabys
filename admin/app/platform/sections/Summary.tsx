...
import { api, money, shortDate, type Me } from '../lib';
...
export default function Summary({ me }: { me: Me }) {
  ...
  const load = async (d = days) => {
    try { setData(await api(`/metrics?days=${d}`)); setErr(''); }
        ...
        {[7, 30, 90].map((d) => (
          <button key={d} onClick={() => { setDays(d); load(d); }}
            style={{
              ...
              border: `1px solid ${days === d ? C.accent : C.line}`,
              background: days === d ? C.accent : C.card,
      ...
      <Card title="Сейчас">
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
          ...
          <Big title="Просрочены" value={String(data.now.expired)}
            tone={data.now.expired > 0 ? C.red : undefined} />
          <Big title="Доход в месяц" value={money(data.now.mrr)} big />
        </div>
        {data.note && (
          <div style={{ fontSize: 13, color: C.amber, marginTop: 10 }}>{data.note}</div>
        )}
      </Card>
          ...
          {data.series.map((d: any) => (
            <div key={d.day} title={`${shortDate(d.day)}: ${money(d.amount)}`}
              style={{
                ...
                background: d.amount > 0 ? C.accent : C.line,
...
function Big({ title, value, big, tone }: {
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: big ? 28 : 22, fontWeight: 600, color: tone ?? C.text,
