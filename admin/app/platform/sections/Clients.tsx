...
export default function Clients({ me }: { me: Me }) {
  ...
  return (
    <>
      {/* ПОДПИСЬ РАЗНАЯ. У партнёра было «Все магазины платформы», а он
          видит только своих. Врущая подпись хуже отсутствующей: он
          решит, что у платформы всего три клиента. */}
      <PageHead title={isSuper ? 'Клиенты' : 'Мои клиенты'}
        sub={isSuper
          ? 'Все магазины платформы: состояние, срок оплаты и кто ведёт.'
          : 'Магазины, которые вы привели: состояние и срок оплаты.'} />

      <div className="page-head">
        <div>
          <p className="muted">{isSuper
            ? 'Все магазины платформы: состояние, срок оплаты и кто ведёт.'
            : 'Магазины, которые вы привели: состояние и срок оплаты.'}</p>
        </div>
        {/* КНОПКА НУЖНА ОБОИМ, но делает разное: владелец ЗАВОДИТ
            магазин, партнёр ПРОСИТ его завести.

            Партнёр не видел её ВОВСЕ — весь блок был закрыт условием
            isSuper. Заявка на сервере работала, а нажать было негде:
            то же, что нашлось в кассе — умеет всё и не показывает
            ничего.

            Учебный магазин остаётся только владельцу: он для показа
            клиенту, партнёру заводить его незачем. */}
