...
# Язык: человек не должен видеть код. Два ответа сервера когда-то
# говорили с владельцем платформы словами «base» и «pos» — в кабинете
# он видит «Старт» и «Касса», и эти слова ему ничего не сказали бы.
if node scripts/check-language.js; then
  :
else
  echo "ОСТАНОВЛЕНО: человек увидит код или слишком длинный ответ."
  exit 1
fi
