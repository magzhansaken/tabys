...
(async () => {
  ...
  // Дописано по сверке с донором: у них 39 методов, у нас было 30.
  {
    ...
    /* ПРАВИЛО СНОВА ИЗМЕНИЛОСЬ, и по делу.

       Партнёр СТОИТ В МАГАЗИНЕ и ставит кассу — ему надо отдать
       владельцу вход прямо сейчас, при нём. Заявка значила уехать ни с
       чем и приехать второй раз.

       Теперь заводит сразу, доступы выходят обоим одинаково. А
       владельцу платформы ложится запись: он видит, кого привели.

       Риска нет: пробный срок 14 дней, денег магазин не приносит, пока
       не оплатит. */
    ...
    // Записан НА ПАРТНЁРА: доля пойдёт ему.
    v = await j('GET', '/platform/clients', null, PARTNER);
    ok((v.d?.rows ?? []).some((x) => x.id === newId),
       '★ Магазин записан на партнёра — доля пойдёт ему');

    /* И ВЛАДЕЛЕЦ ПЛАТФОРМЫ ВИДИТ, кого привели: иначе партнёр заведёт
       десять пустых и покажет их как работу. */
    v = await j('GET', '/platform/requests', null, SUPER);
    const записи = Array.isArray(v.d) ? v.d : (v.d?.rows ?? []);
    ok(записи.some((x) => x.kind === 'new_tenant' && x.account_id === newId),
       '★ Владельцу платформы легла запись: кого привёл партнёр');

    /* ДУБЛЬ ЛОВИТСЯ РАНЬШЕ И ТОЧНЕЕ.
       Партнёр заводит магазин сразу, значит второй раз с тем же
       телефоном отбивает проверка занятого номера — и НАЗЫВАЕТ, у
       какого магазина он уже стоит.
       Это лучше прежнего «заявка уже отправлена»: партнёр видит имя
       магазина и может спросить, не он ли это. */
    {
      const тел = '+77014445566';
      v = await j('POST', '/platform/tenants',
        { name: 'Дважды', ownerName: 'Кто', ownerPhone: тел }, PARTNER);
      ok(!!v.d?.id, 'Первый магазин заведён');

      v = await j('POST', '/platform/tenants',
        { name: 'Он же', ownerName: 'Кто', ownerPhone: тел }, PARTNER);
      ok(v.status >= 400 && /уже у магазина «Дважды»/.test(v.d?.message ?? ''),
         `★ Тот же телефон отбит и НАЗВАН магазин: «${v.d?.message}»`);
    }
  ...
  // ── СПИСОК ПАРТНЁРОВ ДЛЯ ПЕРЕДАЧИ КЛИЕНТА ─────────────────────
  //
  // Дописано по замечанию: лист «Передать другому партнёру» показывал
  // «Доля с будущих оплат: undefined%».
  //
  // Список партнёров в разделе клиентов отдавал только имя и ключ —
  // доли в нём не было. А лист её показывает, потому что от неё
  // зависит, во что обойдётся передача.
  {
    const v = await j('GET', '/platform/clients', null, SUPER);
    const list = v.d?.partners ?? [];
    ok(list.length > 0, `Список партнёров для передачи не пуст: ${list.length}`);
    ok(list.every((p) => typeof p.commissionPercent === 'number'),
       '★ У каждого партнёра в списке есть доля — иначе в листе «undefined%»');
    ok(list.every((p) => p.id && p.name),
       '★ И ключ с именем: по ключу передают, имя человек читает');
  }

  // ── РЕЙТИНГ ПАРТНЁРОВ СО ШКАЛОЙ ЗВЕРЕЙ ────────────────────────
  //
  // Взято у донора дословно: восемь зверей от крысы до орла, место по
  // числу ПЛАТЯЩИХ клиентов, зверь по ДОЛЕ в списке.
  //
  // Отличие одно: у донора рейтинг лежит в сводке, а сводка закрыта
  // партнёру целиком — соревнование видит только тот, кто в нём не
  // участвует. У нас партнёр видит, но чужие имена и суммы скрыты.
  {
    const v = await j('GET', '/platform/ranking', null, SUPER);
    const rows = v.d?.rows ?? [];
    ok(rows.length > 0, `★ Рейтинг партнёров считается: строк ${rows.length}`);

    if (rows.length) {
      // Первое место — орёл: у донора верхний зверь всегда достаётся
      // первому, сколько бы участников ни было.
      ok(rows[0].animal?.includes('Орёл'),
         `★ Первое место — орёл: «${rows[0].animal}»`);
    ...
    const mine = await j('GET', '/platform/ranking', null, PARTNER);
    const rowsP = mine.d?.rows ?? [];
    ok(mine.status === 200,
       '★ Партнёру рейтинг ОТКРЫТ — у донора он закрыт даже участникам');

    const me = rowsP.find((r) => r.isMe);
    const others = rowsP.filter((r) => !r.isMe);
    ok(!me || !!me.name, '★ Свою строку партнёр видит с именем');
    ok(others.every((r) => r.name === null),
       `★ Чужие имена скрыты: ${others.length} строк без имени`);
    ok(others.every((r) => r.mrr === null),
       '★ Чужой доход скрыт — это чужие деньги');
    ok(others.every((r) => r.animal && r.paid !== undefined),
       '★ Но зверь и число платящих видны — иначе не понять, насколько отстал');
  }

  // ── РЕКВИЗИТЫ СЛОВАМИ ДОХОДЯТ ДО КЛИЕНТА ──────────────────────
  //
  // Дописано в описи полей настроек. Поле «реквизиты словами» уже
  // отдавалось клиенту в его кабинет — а ЗАДАТЬ его было негде: в базе
  // есть, сервер читает и показывает, но всегда пустым.
  //
  // Нужно для банковского счёта: БИН, КБе, номер счёта не влезают в
  // «получателя» и «номер телефона». Магазин-ТОО платит именно так.
  {
    const details = 'ТОО «Табыс»\nБИН 123456789012\nСчёт KZ12 3456 7890';
    const r = await j('POST', '/platform/pay-settings', { payDetails: details }, SUPER);
    ok(r.status < 300, 'Реквизиты словами сохраняются');

    const back = await j('GET', '/platform/pay-settings', null, SUPER);
    ok(back.d?.payDetails === details,
       '★ Реквизиты словами читаются обратно — их есть где задать');
  }

  // ── НЕУДАЧНЫЙ ВХОД ОСТАВЛЯЕТ СЛЕД ─────────────────────────────
  //
  // Дописано в описи действий. Проверил все 31 меняющее действие —
  // тридцать пишутся в журнал. Не писался только ВХОД.
  //
  // Успешный и не надо: он бывает по десять раз в день и засорит след,
  // да и виден столбцом «Был в системе». А неудачный не виден нигде:
  // кто-то подбирает пароль к панели, где все деньги платформы, и
  // владелец узнает об этом только когда вход УДАСТСЯ. То есть поздно.
  {
    const before = (await j('GET', '/platform/audit?weight=access&limit=50',
      null, SUPER)).d?.rows?.length ?? 0;

    await j('POST', '/platform/login', { email: mail, password: 'заведомо-неверный' });

    const after = (await j('GET', '/platform/audit?weight=access&limit=50',
      null, SUPER)).d?.rows ?? [];
    ok(after.length > before,
       '★ Неудачная попытка входа оставляет след');
    ok(after.some((r) => r.action === 'login_failed'),
       '★ Попытка видна во вкладке «доступ» — там её и ищут');

    // Успешный вход журнал не засоряет.
    const all = (await j('GET', '/platform/audit?limit=100', null, SUPER)).d?.rows ?? [];
    ok(!all.some((r) => r.action === 'login_ok' || r.action === 'login'),
       '★ Успешный вход не засоряет журнал — он бывает десять раз в день');
  }
  ...
  // ── КЛЮЧ ЧЕКА: РАСТЁТ ПО ВРЕМЕНИ И ПРИНИМАЕТСЯ СЕРВЕРОМ ───────
  //
  // Часть 14 разбора их кассы. Их правило: «сервер идемпотентен по
  // eventId, поэтому отправить ещё раз безопасно».
  //
  // Я перевёл ключи кассы на ULID и СЛОМАЛ ОТПРАВКУ: поля в базе типа
  // uuid, а ULID туда не лезет. Ни один чек не ушёл бы — касса копила
  // бы их, а отправка падала с ошибкой.
  //
  // Проверка держит оба свойства разом: вид UUID и повтор безопасен.
  {
    // Ключ той же выделки, что у кассы: UUIDv7.
    let ms = 0, sq = 0;
    const uuid7 = () => {
      const n = Date.now();
      if (n === ms) sq = (sq + 1) & 0xfff; else { ms = n; sq = 0; }
      const t = n.toString(16).padStart(12, '0');
      const q = sq.toString(16).padStart(3, '0');
      const r = (k) => Array.from({ length: k },
        () => '0123456789abcdef'[Math.floor(Math.random() * 16)]).join('');
      return `${t.slice(0, 8)}-${t.slice(8, 12)}-7${q}-`
        + `${'89ab'[Math.floor(Math.random() * 4)]}${r(3)}-${r(12)}`;
    };

    const shape = /^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;
    const keys = Array.from({ length: 500 }, uuid7);

    ok(keys.every((k) => shape.test(k)),
       '★ Ключ чека имеет вид UUID — сервер такой примет');
    ok(new Set(keys).size === keys.length,
       '★ Ключи не повторяются: 500 подряд');
    ok(keys.join('') === keys.slice().sort().join(''),
       '★ И растут по времени — чеки в журнале лягут по порядку сами');
  }
