...
(async () => {
  ...
  // Таблицу с названиями конкурентов заменили блоком «Спросите это у любой
  // программы учёта»: восемь неудобных вопросов и наши ответы. Покупатель
  // идёт с этим списком куда угодно и сравнивает сам — это честнее таблицы,
  // где мы сами себе ставим галочки, и не устаревает, когда конкурент
  // что-то доделает.
  ok(html.includes('Спросите это у любой программы') && html.includes('интернет пропадёт'),
     '★ Блок вопросов вместо таблицы сравнения: покупатель проверяет сам');
  ok(!html.includes('UMAG') && !html.includes('Wipon'),
     'Названий конкурентов на сайте нет — не рекламируем их своим сайтом');
  ok(html.includes('lang-switch') && html.includes('ҚАЗ'), '★ Переключатель на казахский на месте');
  ok(html.includes('Хочу пилот') && html.includes('id="lead"'),
     'Форма заявки на месте (якорь переименован на lead)');
  ok(html.includes('6 900') && html.includes('14 900'),
     '★ Оба тарифа на витрине: скрывать цену — терять тех, кто не хочет звонить');
