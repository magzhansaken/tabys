...
export class PlatformService {
  ...
  async setPriceBook(ctx: PlatformCtx, d: any) {
    ...
    return { ok: true, note: 'Цена изменена для всех на этом тарифе. Оплаченное время остаётся: клиент доработает купленный срок, а следующий счёт будет по новой цене.' };
  }
  ...
  async deviceAdd(ctx: PlatformCtx, accountId: string, kind: 'pos' | 'store') {
    ...
    if (pv.proRata > 0) {
      // НАЗВАНИЕ ДОЛЖНО ОБЪЯСНЯТЬ ЦИФРУ. Было «Касса 2 · остаток
      // периода» и сумма 4 400 при цене кассы 3 000 — человек решал,
      // что с него берут за тариф второй раз.
      //
      // Пишем прямо: за сколько дней и почём. «Касса 2 · 44 дн. по
      // 3 000 ₸/мес» — цифра перестаёт быть загадкой.
      await this.q(`SELECT platform_charge_add($1,$2,$3,$4)`,
        [accountId,
         `${made?.device_name ?? 'Устройство'} · ${pv.daysLeft} дн. по ${money(unit)} ₸/мес`,
  ...
  async createTenant(ctx: PlatformCtx, d: {
    ...
    withDemo?: boolean;
    /** Слово партнёра владельцу платформы: откуда клиент. */
    comment?: string;
  }) {
    const name = String(d.name ?? '').trim();
    if (!name) throw new BadRequestException('Укажите название магазина');
    ...
    /* ЗАПИСЬ ДЛЯ ВЛАДЕЛЬЦА ПЛАТФОРМЫ, когда завёл ПАРТНЁР.
     *
     * Магазин уже работает — партнёр отдал доступы владельцу магазина
     * при нём. Но владелец ПЛАТФОРМЫ должен знать, кого привели: иначе
     * партнёр заведёт десять пустых и покажет их как работу.
     *
     * Запись ложится в тот же ящик заявок, откуда он их и решает.
     * Магазин при этом торгует: пробный срок идёт. */
    if (ctx.role === 'partner') {
      await this.q(
        `INSERT INTO tenant_request (account_id, kind, payload, comment, created_by)
         VALUES ($1, 'new_tenant', $2, $3, $4)`,
        [acc.id,
         JSON.stringify({ name, ownerPhone: d.ownerPhone,
           ownerName: d.ownerName, trialDays: Math.floor(trial), byPartner: true }),
         d.comment ?? null, ctx.userId]).catch(() => {
           /* Не роняем заведение из-за записки: магазин уже работает,
              а запись владелец увидит в списке клиентов. */
         });
      await this.audit(ctx, 'tenant_by_partner', acc.id, { name });
    }
...
export class PlatformController {
  ...
  async createTenant(@Req() r: any, @Body() d: any) {
    ...
    /* ПАРТНЁР ЗАВОДИТ СРАЗУ, А ВЛАДЕЛЕЦ ПЛАТФОРМЫ ПОДТВЕРЖДАЕТ.
     *
     * Было: партнёр слал заявку, магазин появлялся после одобрения. Но
     * партнёр СТОИТ В МАГАЗИНЕ и ставит кассу — ему надо отдать
     * владельцу вход прямо сейчас, при нём. Ждать одобрения значит
     * уехать ни с чем и приехать второй раз.
     *
     * Теперь заводится сразу: доступы выходят и партнёру, и владельцу
     * платформы одинаково. А в «Ждут подтверждения» ложится запись —
     * владелец платформы видит, кого привёл партнёр, и подтверждает.
     *
     * Риска нет: пробный срок 14 дней, денег магазин не приносит, пока
     * не оплатит. Не подтвердили — встанет сам. */
