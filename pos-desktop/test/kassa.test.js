...
(async () => {
  ...
  // ── ПЕЧАТЬ ПО ОЧЕРЕДИ ─────────────────────────────────────────
  //
  // Их правило, выстраданное кассовой практикой: «Чек, бегунок кухни и
  // бегунок бара могут прилететь в одну секунду. Если слать их
  // параллельно, задания перемешиваются и ЛЕНТА РВЁТСЯ ПОСРЕДИ ЧЕКА».
  //
  // В магазине то же: кассир пробил чек, тут же нажал «повторить
  // печать», а фискальный ответ печатает свою строку. Три задания в
  // одну секунду — и покупатель получает половину своего чека и
  // половину чужого.
  console.log('\n═══ ПЕЧАТЬ ПО ОЧЕРЕДИ ═══\n');
  {
    ...
    const trace = [];
    const job = (name, ms) => () => new Promise((r) => {
      trace.push('начал ' + name);
      setTimeout(() => { trace.push('кончил ' + name); r(); }, ms);
    });
    ...
    let mixed = false, open = false;
    for (const t of trace) {
      if (t.startsWith('начал')) { if (open) mixed = true; open = true; }
      else open = false;
    }
    ok(!mixed, '★ Три задания разом печатаются строго по одному');
    ok(trace.length === 6, 'Все три дошли до конца');
  }

  // ── ОТКАТ К СТАРОЙ ВЕРСИИ ─────────────────────────────────────
  //
  // Владелец выложил 2.5.0, потом по ошибке залил 2.3.0 поверх — и
  // кассы поставили бы СТАРУЮ, откатив правки недели разом. Хуже:
  // касса уже писала чеки в новом виде, а старая их не поймёт.
  console.log('\n═══ ОТКАТ ВЕРСИИ ═══\n');
  {
    const isNewer = (a, b) => {
    ...
    // Сравнение ЧИСЛАМИ, а не строками: «2.10» строкой меньше «2.9».
    ok(isNewer('2.10.0', '2.9.0'), '★ 2.10 новее 2.9 — сравниваем числами, не строками');
  }
