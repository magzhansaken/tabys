...
/* UUID, РАСТУЩИЙ ПО ВРЕМЕНИ — правило донора, но в записи, которую
 * принимает сервер.
 *
 * Сперва я взял ULID, как у них, и СЛОМАЛ ОТПРАВКУ: поля ключей в базе
 * типа uuid, а ULID туда не лезет. Ни один чек не ушёл бы на сервер —
 * касса копила бы их в очереди, а отправка падала с ошибкой.
 *
 * Урок тот же, что в части 10: поменял запись — сверь ВЕСЬ путь, а не
 * только то место, где правил.
 *
 * UUIDv7 даёт и то и другое: первые двенадцать знаков — время в
 * миллисекундах, остальное случайно. Вид обычный, сервер принимает, а
 * чеки в журнале ложатся ПО ПОРЯДКУ сами — при разборе спорного дня их
 * не надо сортировать.
 */
let uuidLastMs = 0;
let uuidSeq = 0;
const uuid = () => {
  // Порядок внутри одной миллисекунды: касса создаёт ключи быстрее,
  // чем тикают часы, и без счётчика порядок решал бы случай.
  const now = Date.now();
  if (now === uuidLastMs) uuidSeq = (uuidSeq + 1) & 0xfff;
  else { uuidLastMs = now; uuidSeq = 0; }

  const t = now.toString(16).padStart(12, '0');           // 48 бит времени
  const seq = uuidSeq.toString(16).padStart(3, '0');      // 12 бит счётчика
  const r = (n) => Array.from({ length: n },
    () => '0123456789abcdef'[Math.floor(Math.random() * 16)]).join('');

  // Вид: TTTTTTTT-TTTT-7SSS-Vrrr-rrrrrrrrrrrr
  // 7 — метка версии, V — метка варианта: их проверяют строгие базы.
...
async function tryPin() {
  ...
  try {
    ...
    // БЕДА НАЗЫВАЕТСЯ ВСЛУХ, А НЕ ПРЯЧЕТСЯ — правило донора из их
    // проверок на прочность.
    //
    // Ошибка глоталась молча. Сети нет и запаса нет — кассир видит
    // ПУСТОЙ каталог и не понимает почему: ни товаров, ни объяснения.
    // Он думает, что касса сломалась, и звонит.
    //
    ...
    await pullCatalog().catch(() => {
      if (!catalog.length) {
        toast('Нет связи и нет сохранённых товаров — касса не сможет продавать. '
          + 'Дождитесь интернета или позовите владельца', true);
      }
    });
    openSale(); syncLoop();
  } catch (e) {
...
$('btnMenu').onclick = () => {
  sheetGrid('Меню', 'смена, деньги, контроль', [
    ...
    // ЖУРНАЛ ПЕЧАТИ — рядом с настройками. Владелец звонит «чеки не
    // печатаются»: говорим «меню → Журнал печати», он присылает файл, и
