...
(async () => {
  ...
  let поправлено = 0;
  let деньгиПравда = 0;
  /* СЧИТАЕМ ОТДЕЛЬНО: найдено и то, что ВПРАВДУ можно поправить.
     Иначе внизу обещаю «уменьшит выручку на 30 ₸», а все найденные
     помечены «руками» — владелец запустит и не увидит перемен. */
  let можно = 0;
  let деньгиМожно = 0;
  ...
  if (!ЧИНИТЬ) {
    ...
    if (!можно) {
      /* НЕЧЕГО ЧИНИТЬ — ГОВОРИМ ПРЯМО, а не предлагаем ключ впустую:
         владелец запустит --fix и не увидит перемен. */
      console.log('  ПОПРАВИТЬ НЕЧЕГО: все найденные помечены «руками».');
      console.log('');
      console.log('  Выручка завышена на ' + деньги(сумма) + '. Разобрать');
      console.log('  можно в кабинете: открыть чек и решить, к какой');
      console.log('  продаже он относится.');
      console.log('');
      process.exit(0);
    }
    ...
    console.log('');
    console.log('  Поправится ' + можно + ' из ' + всего
      + ', выручка уменьшится на ' + деньги(деньгиМожно) + ' — и это');
    console.log('  ВЕРНО: деньги покупателям отданы, приходом они не были.');

    if (можно < всего) {
      console.log('');
      console.log('  Остальные ' + (всего - можно) + ' на '
        + деньги(сумма - деньгиМожно) + ' названы выше — их руками.');
    }
    console.log('');
    process.exit(0);
  }
  ...
  if (!поправлено) {
    console.log('  НЕ ПОПРАВЛЕНО НИЧЕГО: все найденные помечены «руками».');
    console.log('');
    console.log('  Выручка завышена на ' + деньги(сумма) + ' — разбирайте');
    console.log('  в кабинете.');
    console.log('');
    process.exit(0);
  }
