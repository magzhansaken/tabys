...
(async () => {
  ...
  console.log('');
  console.log('  ' + 'Магазин'.padEnd(24) + 'Чеков  Выручка      Товаров  Заведён  Признак');
  ...
  for (const м of магазины) {
    ...
    const п = (await c.query(
      `SELECT
         ...
         (SELECT count(*) FROM product WHERE deleted_at IS NULL
            AND coalesce(article,'') <> 'УЧЕБНЫЙ')::int товаров,
    ...
    const пусто = п.чеков === 0 && п.товаров === 0;
    ...
    console.log('  ' + м.name.slice(0, 23).padEnd(24)
      ...
      + (пусто ? 'ни торговли, ни своих товаров' : ''));
  }
