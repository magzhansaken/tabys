...
const RULES = [
  ...
  { file: 'deploy/12_deploy_git.sh', must: 'node scripts/check-routes.js',
    why: 'Сторож адресов в выкладке: кнопка, зовущая несуществующий путь, падает молча — ни сборка, ни тесты этого не видят' },

  { file: 'server/src/platform/platform.module.ts', must: 'Срез пишется ДВУМЯ путями',
    why: 'Планировщик пишет день без панели, открытие сводки страхует его: заполнить дырку в истории потом нечем' },

  { file: 'server/src/platform/platform.module.ts', must: 'ПРОВЕРКИ ЖИВУТ ЗДЕСЬ, а не только в кабинете',
    why: 'Скидка 150% давала клиенту отрицательный счёт, цена ноль обнуляла доход со всех разом — сервер не проверял ничего' },
  { file: 'server/src/platform/platform.module.ts', must: 'card_updated',
    why: 'Правка карточки пишется в журнал: спросить «кто сменил телефон клиента» было некого' },

  { file: 'server/src/platform/platform.module.ts', must: 'Реквизиты видит владелец платформы',
    why: 'Партнёр не видит реквизиты: клиенты платят напрямую платформе, чужой счёт ему знать незачем' },
  { file: 'server/test/part46.platform.e2e.js', must: '★ Доля платформы скрыта от партнёра',
    why: 'Разграничение проверяется тестом: обе утечки не ломали работу и потому не всплывали сами' },

  { file: 'db/migrations/071_search_fix.sql', must: "phone_tail(p_q) <> ''",
    why: 'По телефону ищем только если в запросе есть цифры: иначе пустой хвост совпадал с пустым и поиск показывал всех' },
  { file: 'db/migrations/072_card_owner.sql', must: 'owner_name, owner_phone, touched_at',
    why: 'Владелец и телефон пишутся в карточку: без них колонка пустует, поиск не находит и звонить некому' },

  { file: 'server/src/platform/platform.module.ts', must: 'Берём у ТОЙ ЖЕ функции, что и список',
    why: 'Карточка считала счёт по-своему: в ней стоял ноль, а в списке рядом 6 900 — восьмое место с тем же расчётом' },
  { file: 'server/test/part46.platform.e2e.js', must: '★ Счёт в карточке и списке совпадает',
    why: 'Одна цифра в двух местах расходилась молча — теперь это стережёт тест' },

  { file: 'server/test/part46.platform.e2e.js', must: '★ Счёт вырос ровно на цену строки',
    why: 'Одобрение должно МЕНЯТЬ ДЕЛО: счёт клиента растёт ровно на цену, названную при решении' },

  { file: 'db/migrations/073_funnel_auto.sql', must: "'auto' СНИМАЕТ ручную отметку",
    why: 'Карточка, двинутая в «Отказ» руками, застревала навсегда: клиент платит, а в воронке архив' },
  { file: 'admin/app/platform/sections/Funnel.tsx', must: 'по фактам «${',
    why: 'Когда факты спорят с ручным этапом, это видно: клиент платит до 2027 года, а карточка в «Отказе»' },

  { file: 'server/src/platform/platform.module.ts', must: 'canActivateAsync',
    why: 'Живость проверяется при каждом обращении: партнёру закрыли вход, а старый ключ работал до конца срока' },
  { file: 'server/test/part46.platform.e2e.js', must: '★ Отключённый партнёр не видит клиентов даже со старым ключом',
    why: 'Отключение партнёра должно действовать сразу, а не после истечения его ключа' },

  { file: 'server/test/part46.platform.e2e.js', must: '★ Снимок за день один',
    why: 'Снимок пишется при каждом открытии сводки: задвоится — график покажет два столбика за один день' },

  { file: 'db/migrations/074_snapshot_split.sql', must: 'ПЛАТЯТ: срок не вышел И была хоть одна подтверждённая',
    why: '«Работают» и «на пробном» пересекались: человек складывал 3+1=4 при трёх магазинах и переставал верить сводке' },
  { file: 'server/test/part46.platform.e2e.js', must: '★ Платят + пробные + просрочены = всего',
    why: 'Цифры сводки должны складываться: иначе человек ищет свою ошибку там, где ошибка в счёте' },

  { file: 'db/migrations/075_journal_cursor.sql', must: 'листаем по нему',
    why: 'Листание по времени теряло записи одной секунды: массовое действие пишет несколько разом' },
  { file: 'server/test/part46.platform.e2e.js', must: '★ Листание журнала ничего не теряет',
    why: 'Журнал — свидетель: пропущенная при листании запись всё равно что стёртая' },

  { file: 'db/migrations/075_journal_cursor.sql', must: 'pa.seq < p_before',
    why: 'Листание журнала по номеру, а не по времени: массовое действие пишет записи одним мгновением, и время их теряло' },
  { file: 'server/test/part46.platform.e2e.js', must: '★ Листание журнала ничего не теряет',
    why: 'Пропавшие записи журнала не видны на глаз: список не пустой, просто части событий в нём нет' },

  { file: 'admin/app/(cab)/billing/page.tsx', must: 'Куда платить',
    why: 'Владелец магазина открывал «Подписку» и не видел реквизитов: сервер их отдавал, страница не спрашивала' },
  { file: 'admin/app/(cab)/billing/page.tsx', must: 'Перевод на:',
    why: 'Номер отдельной строкой: слитую строку копируют целиком и вставляют в поле номера — перевод не проходит' },

  { file: 'db/migrations/076_ghost_partner.sql', must: 'Партнёр не найден или удалён',
    why: 'Клиент не числится за призраком: в списке «ведёт Ерлан», а в отборе Ерлана нет — таких клиентов не выбрать ничем' },
  { file: 'admin/app/platform/sections/Summary.tsx', must: 'Клиентов пока нет',
    why: 'Первый день: графики из нулей дают прямую линию, и человек не понимает — сломалось или данных нет' },

  { file: 'server/src/platform/platform.module.ts', must: 'Телефон владельца непохож на номер',
    why: 'Телефон — это ВХОД владельца: «не телефон» принимался молча, магазин заводился, а войти по такому нельзя' },
  { file: 'server/src/platform/platform.module.ts', must: 'ПРОВЕРКИ ТЕ ЖЕ, ЧТО ПРИ ЗАВЕДЕНИИ',
    why: 'Правка карточки не проверяла ничего: испортить можно было то, что не пропустили при создании' },

  { file: 'server/src/platform/platform.module.ts', must: 'Срок оплаты — от 1 до 36 месяцев',
    why: 'Срок молча чинился: «0» и «−3» становились единицей, «999» проходил и продлевал клиента до 2109 года' },
  { file: 'server/src/platform/platform.module.ts', must: 'Отрицательная цена бывает только у скидки',
    why: 'Минус у обычной строки уменьшал счёт, хотя человек думал, что добавляет плату' },

  { file: 'server/src/platform/platform.module.ts', must: 'СОДЕРЖИМОЕ заявки проверяем ЗДЕСЬ',
    why: '«Просит вертолёт» доходил до владельца, и тот получал ошибку базы вместо ответа — а ошибся партнёр неделю назад' },
  { file: 'server/test/part46.platform.e2e.js', must: '★ Гонка решений отбита',
    why: 'Два решения по одной заявке разом задвоили бы строку счёта: клиент платил бы за одну кассу дважды' },

  { file: 'db/migrations/077_stage_list.sql', must: 'три разных списка этапов',
    why: 'Списков этапов было три и они расходились: обычный сдвиг в «Пробный» падал ошибкой базы' },
  { file: 'server/test/part46.platform.e2e.js', must: '★ Все пять этапов воронки работают',
    why: 'Каждый этап проверяется отдельно: расхождение списков видно только при ручном сдвиге' },

  { file: 'server/src/platform/platform.module.ts', must: 'Почта непохожа на адрес',
    why: 'По почте партнёр входит и получает письма: «непочта» принималась молча — войти можно, написать нельзя' },
  ...
  { file: 'server/src/platform/platform.module.ts', must: 'только адрес, начинающийся с https',
    why: '«javascript:» в кнопке «Оплатить» — чужой код в кабинете владельца магазина, который жмёт её, доверяя платформе' },
  ...
  { file: 'admin/app/platform/style/admin.css', must: 'Длинные строки без пробелов переносим по знакам',
    why: 'Почта партнёра и код привязки растягивали карточку за край экрана: в них нечему переноситься' },

  { file: 'admin/app/platform/ui/Chart.tsx', must: 'ПОДПИСИ ЖИВУТ ВНЕ ХОЛСТА',
    why: 'Холст растягивается по ширине, и текст внутри него сплющивался вдвое — на телефоне цифры нечитаемы' },
  ...
  { file: 'db/migrations/079_who_declared.sql', must: 'Кто отметил оплату',
    why: 'Клиент нажал «Я оплатил», а в ленте стояло имя партнёра: по спорной оплате пошли бы не к тому' },
  { file: 'admin/app/(cab)/billing/page.tsx', must: 'Реквизиты пока не указаны',
    why: 'Без реквизитов блок просто не показывался: человек не понимал, забыли или надо звонить' },
  ...
  { file: 'server/src/platform/platform.module.ts', must: 'ИТОГ ДНЯ ДЛЯ ПАРТНЁРА',
    why: 'Подтверждённая оплата уходит из ленты, и партнёр не узнавал, что доля начислена' },

  { file: 'admin/app/platform/sections/Clients.tsx', must: 'срок не вышел, включая пробных',
    why: '«Работают 3» в клиентах и «Платят 2» в сводке — разные вещи, и без подписи человек решит, что одна врёт' },

  { file: 'db/migrations/081_journal_readable.sql', must: 'ЧТО ИМЕННО ПОПРАВИЛИ в карточке',
    why: '«Правка карточки» — самая частая запись, и по ней ничего не восстановить через полгода' },
  ...
  { file: 'server/test/part46.platform.e2e.js', must: '★ Ряд и итог периода сходятся',
    why: 'Ряд сводки собирается из снимков и живых оплат: разойдутся — в сводке одно, в «Деньгах» другое' },

  { file: 'server/src/platform/platform.module.ts', must: 'НЕУДАЧНАЯ ПОПЫТКА — В ЖУРНАЛ',
    why: 'Подбор пароля к панели проходил молча: владелец узнал бы, только когда вход удался' },

  { file: 'admin/app/platform/sections/Settings.tsx', must: 'Реквизиты для перевода со счёта',
    why: 'Поле отдавалось клиенту, а задать его было негде: магазин-ТОО платит со счёта, БИН в «получателя» не влезает' },

  { file: 'db/migrations/087_partner_ranking.sql', must: "'🐀 Крыса','🐌 Улитка','🐹 Хомяк','🐰 Заяц'",
    why: 'Шкала зверей взята у донора дословно: порядок важен, первый — последнее место' },
  ...
  { file: 'admin/app/platform/ui/InlineText.tsx', must: 'ТО ЖЕ ЗНАЧЕНИЕ — говорим об этом',
  ...
  { file: 'deploy/15_demo_data.sh', must: 'СТЕРЕТЬ',
    why: 'Скрипт данных проверки требует слово-подтверждение: команду вытаскивают из истории, а данные обратно не вытащишь' },
  ...
  { file: 'deploy/16_wipe.sh', must: 'СТЕРЕТЬ-ВСЁ',
    why: 'Очистка требует слово-подтверждение: восстановить будет нечем' },
  { file: 'db/migrations/092_wipe_preview.sql', must: 'Врать перед необратимым действием нельзя',
    why: 'Скрипт показывал «магазинов: 0» перед тем, как стереть одиннадцать: защита строк не пускала к подсчёту' },

  { file: 'admin/app/platform/style/admin.css', must: 'Доступы: подпись слева, значение по центру',
    why: 'Подпись стояла над значением: три доступа занимали шесть строк, и сверять их глазом было неудобно' },

  { file: 'db/migrations/093_paid_beats_manual.sql', must: 'ОПЛАТА СИЛЬНЕЕ РУКИ',
    why: 'После подтверждения оплаты карточка оставалась там, куда её двигали руками — воронка врала' },

  { file: 'server/src/platform/platform.module.ts', must: 'МЕНЯЕМ И ПОДПИСКУ, а не только строку счёта',
    why: 'Тариф менялся в строках, подписка оставалась старой: подпись не менялась, кнопка не переключалась, оплата брала старую цену' },
  { file: 'admin/app/platform/ui/PlanLines.tsx', must: '«Основы» тут нет нарочно',
    why: 'Основа появляется сама при смене тарифа: заведёшь руками — станет две, и счёт вырастет вдвое' },

  { file: 'server/src/platform/platform.module.ts', must: 'ИЗ ЧЕГО СКЛАДЫВАЕТСЯ МЕСЯЦ',
    why: 'Итог отвечает на «сколько», но не на «за что»: тариф и устройства — разные разговоры с клиентом' },
  { file: 'server/src/platform/platform.module.ts', must: 'Знак ставим ОДИН РАЗ, по величине',
    why: 'Вписанная минусом скидка переворачивалась второй раз и становилась надбавкой' },

  { file: 'db/migrations/094_device_add.sql', must: 'добавить устройство целиком, а не полдела',
    why: 'Добавлялась только строка счёта: деньги за кассу брали, а кассы не появлялось — привязывать нечего' },
  { file: 'db/migrations/094_device_add.sql', must: 'ТОЛЬКО если устройство платное',
    why: 'Цена ноль — устройство есть, денег нет: касса в подарок за год или на замену сломанной' },

  { file: 'db/migrations/096_request_device.sql', must: 'ЗАВОДИМ САМО УСТРОЙСТВО',
    why: 'Одобрение заявки растило счёт, а кассы не появлялось: клиент платил за то, чего у него нет' },

  { file: 'db/migrations/098_device_request.sql', must: 'ПАРТНЁР вправе СНИЗИТЬ',
    why: 'Это его доля — он уступает из своего. Поднять нельзя: клиент увидит цену выше платформенной' },
  { file: 'db/migrations/098_device_request.sql', must: 'ВЛАДЕЛЕЦ МАГАЗИНА цену не выбирает',
    why: 'Он покупатель, а не продавец: видит прайс платформы и просит по нему' },

  { file: 'db/migrations/099_partner_sees_requests.sql', must: 'ПАРТНЁР ВИДИТ ЗАЯВКИ СВОИХ КЛИЕНТОВ',
    why: 'Владелец магазина подавал сам, и его партнёр заявки не видел — решали мимо того, кто ведёт клиента' },
  { file: 'admin/app/(cab)/billing/page.tsx', must: 'Прошу ещё кассу',
    why: 'Раньше владелец магазина мог только позвонить: звонок теряется, а заявка остаётся' },

  { file: 'db/migrations/101_code_for_register.sql', must: 'код для НУЖНОЙ кассы, а не всегда первой',
    why: 'Со второй кассой планшет привязывался к старой, и две кассы спорили за одно рабочее место' },
  { file: 'db/migrations/100_device_list.sql', must: 'только если он ЖИВОЙ',
    why: 'Просроченный код человек продиктует, а он не подойдёт — и виноватой окажется система' },

  { file: 'db/migrations/102_charges.sql', must: 'ЛОЖИТСЯ В ОЖИДАНИЕ',
    why: 'Доплату обещали и никуда не клали: платформа дарила остаток месяца и не замечала' },
  { file: 'db/migrations/102_charges.sql', must: 'КАЖДЫЙ МЕСЯЦ, а доплата разовая',
    why: 'Смешать их значит брать разовую доплату вечно' },

  { file: 'db/migrations/103_first_free.sql', must: 'ПЕРВОЕ устройство каждого',
    why: 'Правило донора: владелец платформы не мог отличить кассу из основы от второй, платной' },
  { file: 'db/migrations/103_first_free.sql', must: 'ЦЕНУ БЕРЁМ ИЗ СТРОКИ СЧЁТА',
    why: 'Иначе в списке устройств одна цифра, а в счёте другая — и непонятно, какой верить' },

  { file: 'db/migrations/104_code_lives.sql', must: 'КОД СРАЗУ, а не по кнопке',
    why: 'Полчаса — это звонок, поиск кассира, установка приложения: не успел, звони заново' },
  { file: 'db/migrations/105_own_line.sql', must: 'СВОЯ строка счёта',
    why: 'Бралась любая строка вида: вторая касса показывала строку третьей, и человек искал в счёте не то' },

  { file: 'server/src/platform/platform.module.ts', must: 'дн. по ${money(unit)}',
    why: 'Доплата 4400 при цене кассы 3000 пугала: человек решал, что за тариф берут второй раз' },
  { file: 'admin/app/(cab)/billing/page.tsx', must: 'за тариф вы уже',
    why: 'Клиент видел доплату больше месячной цены устройства и думал, что платит за тариф дважды' },

  { file: 'pos-desktop/renderer/app.js', must: 'ВХОДИТ ЛЮБОЙ ИЗ РАБОТАВШИХ',
    why: 'Кассиры сменяются: пускать только последнего значит заставить вечернего торговать под именем утреннего' },
  { file: 'pos-desktop/renderer/app.js', must: 'КЛЮЧ КАССИРА, А НЕ ТОЛЬКО ИМЯ',
    why: 'Две Айгуль за год — обычное дело: без ключа выручку по кассирам не сверить' },
  { file: 'pos-desktop/renderer/app.js', must: 'ЗАПЕРЕТЬ КАССУ',
    why: 'Выйти можно было только закрыв смену: касса оставалась открытой под чужим именем' },

  { file: 'pos-desktop/renderer/i18n.js', must: 'ЧЕК ОСТАЁТСЯ НА РУССКОМ',
    why: 'Чек читает покупатель и налоговая, а не кассир: язык кассы на него не влияет' },

  { file: 'pos-desktop/electron/main.cjs', must: 'ПОЛНЫЙ ЭКРАН НЕ ВЫКЛЮЧАЕТСЯ',
    why: 'F11 выпускал окно, появлялась рамка с крестиком: кассир задел клавишу и закрыл кассу при очереди' },
  ...
  { file: 'pos-desktop/renderer/app.js', must: 'СВОЙ КОД — четвёртый уровень',
    why: 'Кассир подтверждает себя, а не зовёт старшего: за отмены спрашивают именно с него' },
  { file: 'pos-desktop/renderer/app.js', must: 'БЕЗ СВЯЗИ КОД СТАРШЕГО НЕ СВЕРИТЬ',
    why: 'Касса давала вводить код впустую: кассир при покупателе трижды набирал и решал, что она сломалась' },

  { file: 'pos-desktop/renderer/app.js', must: 'СНЯТЬ ЛИШНИЕ МАРКИ',
    why: 'Убрали бутылку — осталось 2 товара и 3 марки: лишняя ушла бы в налоговую как проданная' },
  { file: 'pos-desktop/renderer/app.js', must: 'ПРЕДЕЛ КОЛИЧЕСТВА',
    why: 'Опечатка в лишнюю цифру давала чек на миллион, и отменять его через старшего' },
  ...
  { file: 'pos-desktop/renderer/app.js', must: 'ГАШЕНАЯ КНОПКА ОБЯЗАНА',
    why: 'Кассир жмёт цифру, ничего не происходит, а почему — неясно: жмёт сильнее, зовёт старшего, звонит' },

  { file: 'pos-desktop/renderer/app.js', must: 'БЫСТРЫЕ СУММЫ ПО ПРАВИЛУ ДОНОРА',
    why: 'Моё правило на 24 000 не показывало ничего: кассир набирал руками при очереди' },

  { file: 'pos-desktop/electron/printer.cjs', must: 'КОПИИ ОДНИМ ЗАДАНИЕМ',
    why: 'Отдельные задания встают в очередь порознь и могут перемешаться с чужим чеком — лента порвётся' },

  { file: 'pos-desktop/renderer/app.js', must: 'НЕ ПЕЧАТАЕТ ЧЕКИ',
    why: 'Печать в PDF принимает задание успешно, бумаги нет: покупатели уходят без чеков весь день' },
  { file: 'pos-desktop/renderer/app.js', must: 'ПОВТОРИТЬ ПЕЧАТЬ',
    why: 'Кончилась лента — чек сохранён, а перепечатать было нечем' },

  { file: 'pos-desktop/renderer/app.js', must: 'ОТКЛОНЁННЫЕ ЧЕКИ — ОТДЕЛЬНО',
    why: 'Отклонённый крутился в очереди вечно: она не пустела, и кассир переставал верить счётчику' },

  { file: 'pos-desktop/renderer/app.js', must: 'ОТЧЁТ ЗАКРЫТИЯ НА БУМАГЕ',
    why: 'Кассир сдаёт по нему деньги старшему: без бумаги сдают на словах, и спор — слово против слова' },
  { file: 'pos-desktop/renderer/app.js', must: 'это не излишек',
    why: 'Кассир видел излишек на сумму неотправленных чеков и искал несуществующую ошибку' },

  { file: 'pos-desktop/renderer/app.js', must: 'ПУСТОЕ ПОЛЕ ДАВАЛО НОЛЬ',
    why: 'Кассир закрывал смену не глядя — в отчёт уходила недостача на всю кассу, которую он не брал' },

  { file: 'pos-desktop/renderer/app.js', must: 'КОД СТАРШЕГО БЕЗ СЕТИ',
    why: 'Сети нет — касса врала «PIN не подошёл», и отменить позицию было нельзя вовсе' },

  { file: 'server/src/pos/pos-settings.module.ts', must: 'ПОМЕТКА «БЕЗ ПРОВЕРКИ» ДОХОДИТ',
    why: 'Интернет можно выключить нарочно: без пометки отмены «без проверки» неотличимы от разрешённых старшим' },

  { file: 'pos-desktop/renderer/app.js', must: 'СВЯЗЬ — ОТДЕЛЬНАЯ ВЕЛИЧИНА',
    why: 'Пустая очередь давала зелёную точку всегда: кассир пробивал чеки при мёртвой сети и не знал' },
  ...
  { file: 'pos-desktop/electron/main.cjs', must: 'ЖУРНАЛ ОБОЛОЧКИ',
    why: 'Владелец звонит «чеки не печатаются» из другого города — без журнала это гадание и поездка' },

  { file: 'server/src/auth/auth.service.ts', must: 'КАССИР УХОДИТ',
    why: 'Явка открывалась и не закрывалась: владелец видит человека на смене, а он ушёл три часа назад' },

  { file: 'pos-desktop/renderer/app.js', must: 'СВОЙ ВОПРОС ВМЕСТО СИСТЕМНОГО',
    why: 'Системное окно Windows крошечное: кассир бьёт пальцем и промахивается мимо «Да»' },
  { file: 'pos-desktop/renderer/index.html', must: 'ПОЛЕ НЕ ЗНАЕТ ФОРМАТ КОДА',
    why: 'Поле принимало только цифры, а код стал TBS-4FE2-3137 — привязка не прошла бы никогда' },

  { file: 'pos-desktop/renderer/app.js', must: 'КАНАРЕЙКА',
    why: 'Касса падала у клиента, кассир перезапускал и молчал — мы не узнавали никогда' },
  { file: 'pos-desktop/renderer/app.js', must: 'ЭКРАН ПОСЛЕ ОПЛАТЫ',
    why: 'Покупатель спрашивает «сколько сдачи», а экран уже пуст — искать в чеках смены' },

  // ── Новая касса: правила, найденные живьём ─────────────────────────
  { file: 'pos2/renderer/core.js', must: 'deviceToken',
    why: 'Ядро искало поле paired — касса привязывалась и оставалась на экране привязки навсегда' },
  { file: 'pos2/renderer/scan.js', must: 'ЗНАК СЪЕДАЕМ',
    why: 'Знаки сканера уходили в поиск, экран перерисовывался на каждой цифре и затирал чек' },
  { file: 'pos2/renderer/screen-sale.js', must: 'sale-cart',
    why: 'Экран продажи не рисовал чек — кассир не видел, что пробил, и не мог отменить' },
  { file: 'pos2/renderer/screen-paid.js', must: 'changeWords',
    why: 'Экран сдачи показывал «undefined» вместо суммы: имена полей расходились с свёрткой' },
  { file: 'pos2/renderer/index.html', must: 'id="paid"',
    why: 'Слоя сдачи не было в разметке — экран после оплаты не показывался вовсе' },
  { file: 'pos2/renderer/refund.js', must: 'уже возвращали',
    why: 'Чек могли принести второй раз — магазин платил бы дважды за один товар' },
  { file: 'pos2/test/page.test.js', must: 'столкновения',
    why: 'Все файлы кассы на одной странице: одно имя в двух местах ломает второй файл целиком' },
  { file: 'deploy/12_deploy_git.sh', must: 'pos2 && npm test',
    why: 'Выкладка без проверок новой кассы выпустит сломанную версию клиентам' },

  { file: 'pos2/renderer/screen-menu.js', must: 'buildMenu',
    why: 'Касса умела всё — отчёты, возвраты, инкассацию — и не показывала ничего: ни одной кнопки' },
  ...
  { file: 'scripts/seed-shop.js', must: 'ВХОД В КАБИНЕТ ВЛАДЕЛЬЦА',
    why: 'Без пароля кабинет недоступен: ни завести товар, ни поменять цену, ни увидеть выручку' },

  { file: 'scripts/seed-shop.js', must: 'ЧИНИМ ВЛАДЕЛЬЦА',
    why: 'Наполнение печатало вход в кабинет, которого нет: человек пойдёт входить и не сможет' },

  { file: 'scripts/check-shop.js', must: 'касса пробьёт их за НОЛЬ',
    why: 'Товар без цены — самая дорогая беда: покупатель унесёт его даром, недостача на кассире' },
  { file: 'scripts/seed-shop.js', must: 'ПРЕДЕЛ СКИДКИ СТАВИМ ЧЕЛОВЕКУ',
    why: 'Роли общие на все магазины: правка предела задела бы чужих клиентов' },

  { file: 'deploy/13_publish_kassa.sh', must: 'ОПИСАНИЕ БЕРЁТСЯ ИЗ ФАЙЛА',
    why: 'PowerShell портит русские буквы: кассир увидел бы мусор вместо того, за что соглашается' },

  { file: 'scripts/seed-shop.js', must: 'ПРИВЯЗКА К МАГАЗИНУ ОБЯЗАТЕЛЬНА',
    why: 'Кассиры заведены, коды стоят, а войти не может никто: сервер ищет только привязанных' },
  ...
  { file: 'pos2/renderer/app.js', must: 'КАТАЛОГ С ДИСКА НЕ СТИРАЕМ',
    why: 'Связь пропала — касса слепла: товары на диске, а на экране пусто и «товар не найден»' },
  ...
  { file: 'server/src/goods/goods.service.ts', must: 'ВЫДАТЬ ШТРИХКОД',
    why: 'Товар без кода сканером не пробить: кассир ищет руками при очереди' },
  { file: 'db/migrations/111_label_60mm.sql', must: 'Наклейка 60×40',
    why: 'Лент не было ни одной: печать наклеек работала, а печатать не на чем' },
  ...
  { file: 'pos2/renderer/age.js', must: 'ПРОВЕРКА ВОЗРАСТА',
    why: 'Продажа табака несовершеннолетнему — штраф до 200 МРП на продавца лично' },
  { file: 'pos2/renderer/cart.js', must: 'ВИД МАРКИРОВКИ НЕСЁМ В СТРОКУ',
    why: 'Без него возраст не спрашивался: пиво уходило без вопроса о документе' },

  { file: 'server/src/platform/platform.module.ts', must: 'ПАРТНЁР ЗАВОДИТ СРАЗУ',
    why: 'Партнёр стоит в магазине: без доступов на месте он уедет ни с чем и приедет второй раз' },
  { file: 'server/src/platform/platform.module.ts', must: 'ЗАПИСЬ ДЛЯ ВЛАДЕЛЬЦА ПЛАТФОРМЫ',
    why: 'Иначе никто не видит, кого привёл партнёр: заведёт десять пустых и покажет как работу' },
  ...
  { file: 'pos2/renderer/lock.js', must: 'ЗАМОК ИДЁТ ТЕМ ЖЕ ПУТЁМ',
    why: 'Замок смотрел только пропуск: касса, заперевшаяся до первого входа, не отпиралась ничем' },

  { file: 'pos2/renderer/common.js', must: 'РАЗВЕРНУТЬ ОТВЕТ ХРАНИЛИЩА',
    why: 'Мост отдаёт обёртку: без развёртки отложенный чек ложился в пустоту, а очередь казалась пустой' },

  { file: 'db/migrations/113_login_reset_on_success.sql', must: 'СЧИТАЕМ ТОЛЬКО ПОСЛЕ ПОСЛЕДНЕГО',
    why: 'Удачный вход не сбрасывал счётчик: владелец входил через раз, хотя пароль верный' },
  { file: 'server/src/auth/auth.service.ts', must: 'СКОЛЬКО ЖДАТЬ — СЧИТАЕМ',
    why: 'Говорили «пять минут», когда ждать оставалась минута: человек ждёт впустую или уходит' },

  { file: 'pos2/renderer/app.js', must: 'ТЕМ ЖЕ ПУТЁМ, КАКИМ ПРИШЛИ',
    why: 'Возврат всегда наличными: ящик пустел, а по карте деньги оставались у магазина' },

  { file: 'admin/app/(cab)/goods/page.tsx', must: 'ТРИ ПОЛЯ ПОДРЯД',
    why: 'Без наценки владелец вводил закуп и не знал, за сколько товар пойдёт: считал в уме' },
  { file: 'admin/app/(cab)/goods/page.tsx', must: 'ТРОНУЛ ЦЕНУ РУКАМИ',
    why: 'Затирать цену владельца значит спорить с ним: он знает про акции и договоры больше наценки' },

  { file: 'scripts/check-all-shops.js', must: 'ПРОВЕРКА ВСЕХ МАГАЗИНОВ РАЗОМ',
    why: 'Беды бывают общими: код кассира не лёг у всех, кто заведён до правки — а узнаём по одному' },

  { file: 'pos2/renderer/sync.js', must: 'ЧЕК В ВИД СЕРВЕРА',
    why: 'Касса шлёт way/cash/card, сервер ждёт payment: чек падал с ошибкой и висел в очереди вечно' },
  { file: 'db/migrations/113_register_warehouse.sql', must: 'СКЛАД У КАССЫ',
    why: 'Без склада чеки уходят в карантин: деньги взяли, а в кабинете продажи нет' },

  { file: 'pos2/renderer/common.js', must: 'ПУСТОТУ ОСТАВЛЯЕМ ПУСТОЙ',
    why: 'Пустой пропуск значит «кода нет»: превратить его в пустой предмет — впустить кого угодно' },
  { file: 'pos2/renderer/passes.js', must: 'МОСТ ОТДАЁТ ОБЁРТКУ',
    why: 'Замок падал на верном коде: пропуск приходил обёрнутым, и кассир оставался запертым' },

  { file: 'pos2/renderer/sync.js', must: 'КАРАНТИН — ЭТО НЕ',
    why: 'Касса считала чек в карантине ушедшим и молчала: деньги взяты, в отчёте пусто' },

  { file: 'scripts/check-receipts.js', must: 'ГДЕ ЧЕКИ',
    why: 'Владелец видит ноль в отчёте и не знает, где чек: в очереди, в карантине или не ушёл' },

  { file: 'pos2/renderer/sync.js', must: 'ИМЕНА СОБЫТИЙ СВОДИМ',
    why: 'Сервер не знал слова refund — возвраты не принимались вовсе, деньги отданы мимо учёта' },
  { file: 'pos2/renderer/sync.js', must: 'МЕТКА ВРЕМЕНИ ОБЯЗАТЕЛЬНА',
    why: 'Без неё смена отбивалась, а за смену цеплялись все чеки: sale_shift_id_fkey' },

  { file: 'server/src/pos/pos.service.ts', must: 'ДЕНЬГИ ВАЖНЕЕ ССЫЛКИ',
    why: 'Чек отбивался целиком из-за недошедшей смены: выручка терялась ради учётной рамки' },
  { file: 'server/src/pos/pos.service.ts', must: 'СТАРЫЕ КАССЫ ЗОВУТ ИНАЧЕ',
    why: 'Сервер не знал refund — возвраты старых касс терялись, а они стоят в магазинах' },
  { file: 'scripts/retry-quarantine.js', must: 'ВЕРНУТЬ ЧЕКИ ИЗ КАРАНТИНА',
    why: 'Чек в карантине — взятые деньги без учёта: тело события цело, применить можно' },

  { file: 'server/src/sync/sync.service.ts', must: 'ПОМЕЧЕННЫЕ, НО НЕ ПРИМЕНЁННЫЕ',
    why: 'Прежний разбор помечал записи и забывал применить: деньги остались вне учёта навсегда' },

  { file: 'server/src/sync/sync.service.ts', must: 'ПРИМЕНЯЕМ ТЕЛО НАПРЯМУЮ',
    why: 'Событие уже в журнале — обычный ход считает повтором и не применяет: деньги вне учёта' },

  { file: 'server/src/sync/sync.service.ts', must: 'ПРОВЕРЯЕМ ПО ТАБЛИЦЕ',
    why: 'Обработчик выходит молча — молчание считалось успехом: «применено 22», а в продажах ноль' },

  { file: 'pos2/renderer/sync.js', must: 'ССЫЛКУ БЕРЁМ ТЕМ ИМЕНЕМ',
    why: 'Возврат ложился как продажа: выручка завышена, владелец заплатит налог с чужих денег' },

  { file: 'scripts/fix-refunds.js', must: 'ОПОЗНАЁМ ТОЧНО',
    why: 'Гадать по сумме значит привязать возврат к чужому чеку и испортить два места разом' },

  { file: 'pos2/renderer/app.js', must: 'ВОЗВРАТ ВОЗВРАТА НЕ ДЕЛАЮТ',
    why: 'Фильтр смотрел не то поле: кассир возвращал возврат и отдавал деньги дважды' },
  { file: 'scripts/fix-refunds.js', must: 'ЦЕПОЧЕК НЕ СТРОИМ',
    why: 'Возврат возврата сервер запрещает: обойти его правкой в базе значит завести кривой учёт' },

  { file: 'scripts/list-shops.js', must: 'ПРИЗНАК ПРОВЕРОЧНОГО',
    why: 'Судить по имени нельзя: «Сат» выглядит случайным, а это может быть настоящий магазин' },

  { file: 'scripts/test-scripts.js', must: 'ПРОВЕРКА САМИХ ПРОВЕРОК',
    why: 'Дважды отдал проверку, врущую на старых данных: она судила по признаку, заведённому позже' },
  { file: 'scripts/fix-refunds.js', must: 'ССЫЛКУ ИЩЕМ ВСЕМИ ИМЕНАМИ',
    why: 'Старая касса звала ссылку returnOf: смотреть одно имя значит не найти возврат' },

  { file: 'scripts/fix-refunds.js', must: 'НЕЧЕГО ЧИНИТЬ — ГОВОРИМ ПРЯМО',
    why: 'Обещал «уменьшит выручку на 30 ₸», а все найденные были помечены «руками» — пустое обещание' },
