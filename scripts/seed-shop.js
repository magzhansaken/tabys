 ...
 *
 * Без пароля кабинет недоступен, а это всё, ради чего он нужен: завести
 * товар, поменять цену, посмотреть выручку, поставить кассиру код.
 *
 * Телефон постоянный: по нему владелец входит, и записывать его надо
 * один раз. Пароль простой нарочно — это проверочный магазин, а не
 * боевой клиент. */
const ТЕЛЕФОН = process.env.SEED_PHONE || '+77010000001';
const ПАРОЛЬ = process.env.SEED_PASSWORD || 'tabys12345';
...
(async () => {
  ...
  if (!acc) {
    ...
    /* ПОРЯДОК ДОВОДОВ: название, ТЕЛЕФОН, имя владельца, пароль.
       Я передавал имя вторым — оно легло в поле телефона, а телефон в
       имя: на боевом сервере вышло «+77010912964 · тел Нурлан
       Абдиров». */
  ...
  for (const [имя, фамилия, pin, роль, предел, должность] of ЛЮДИ) {
    ...
    /* ПРИВЯЗКА К МАГАЗИНУ ОБЯЗАТЕЛЬНА.
     *
     * Найдено живьём: сервер ищет на кассе только тех, кто привязан к
     * магазину. Владелец проходит мимо этой проверки, остальные НЕТ.
     *
     * Кассиры были заведены, коды стояли, а войти не мог никто:
     * «Неверный PIN» на верный код — и не догадаться почему. */
    {
      const кто = есть || (await c.query(
        `SELECT id FROM employee WHERE account_id=$1 AND first_name=$2 AND last_name=$3 LIMIT 1`,
        [A, имя, фамилия])).rows[0];
      if (кто && store) {
        await c.query(
          ...
          [A, кто.id, store.id]).catch(() => {});
      }
    }

    /* ПРЕДЕЛ СКИДКИ СТАВИМ ЧЕЛОВЕКУ, А НЕ РОЛИ.
     *
     * Найдено проверкой: роли ОБЩИЕ на все магазины — правка предела
     * задела бы всех клиентов разом, и кассир чужого магазина получил
     * бы наш предел.
     *
     * У сотрудника есть своё поле, и оно сильнее ролевого: так и
     * задумано — право человека сильнее уклада. */
    if (предел != null) {
      const кто = есть || (await c.query(
        `SELECT id FROM employee WHERE account_id=$1 AND first_name=$2 AND last_name=$3 LIMIT 1`,
        [A, имя, фамилия])).rows[0];
      if (кто) {
        await c.query(
          `UPDATE employee SET discount_limit_pct=$1 WHERE id=$2`,
          [предел, кто.id]).catch(() => {});
      }
    }
  }
  ...
  console.log('');
  console.log('  ВХОД В КАБИНЕТ ВЛАДЕЛЬЦА:');
  console.log('    телефон: ' + ТЕЛЕФОН);
