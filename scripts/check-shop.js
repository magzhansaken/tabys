...
(async () => {
  ...
  /* ПРИВЯЗКА К МАГАЗИНУ — самая скрытая беда. Кассир заведён, код
     стоит, а сервер его не ищет: «Неверный PIN» на ВЕРНЫЙ код, и
     догадаться невозможно.
     Владелец проходит мимо этой проверки, остальные нет. */
  const непривязан = (await c.query(
    `SELECT count(*)::int AS n FROM employee e
      WHERE e.account_id=$1 AND e.is_active AND NOT e.is_owner
        AND NOT EXISTS (SELECT 1 FROM employee_store es WHERE es.employee_id=e.id)`,
    [A])).rows[0];
  ok(непривязан.n === 0, 'все кассиры привязаны к магазину',
     `${непривязан.n} не привязаны — получат «Неверный PIN» на верный код`);
