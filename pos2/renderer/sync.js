...
function чекДляСервера(r) {
  ...
  return {
    ...
    /* ССЫЛКУ БЕРЁМ ТЕМ ИМЕНЕМ, КОТОРЫМ ЗОВЁТ КАССА.
     *
     * Найдено на боевом сервере: возвраты легли как ОБЫЧНЫЕ ПРОДАЖИ.
     * Свёртка возврата зовёт ссылку «ofReceiptId», а я искал
     ...
     * которых не получал.
     *
     * «Возвратность» несёт ССЫЛКА, а не знак минус — так решил сервер,
     * и это верно: иначе каждый отчёт обязан помнить про abs(). */
    refundOf: r.ofReceiptId || r.refundOf || r.returnOf || null,
  };
}
