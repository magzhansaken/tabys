...
// ── ССЫЛКА ВОЗВРАТА НА ИСХОДНЫЙ ЧЕК ───────────────────────────────
{
  /* НАЙДЕНО НА БОЕВОМ СЕРВЕРЕ: возвраты легли как ОБЫЧНЫЕ ПРОДАЖИ.
   *
   * Свёртка возврата зовёт ссылку «ofReceiptId», а перевод искал
   * «refundOf» — и она терялась.
   *
   * Выручка оказывалась ЗАВЫШЕНА: возврат считался продажей, хотя
   * деньги покупателю отданы. Владелец заплатил бы налог с денег,
   * которых не получал. */
  const { чекДляСервера } = require('../renderer/sync.js');
  const { buildRefund, planRefund } = require('../renderer/refund.js');

  const чек = { id: 'r1', number: 5, way: 'cash',
    items: [{ id: 'l1', productId: 'g1', name: 'Айран', qty: 4, price: 220, discount: 0 }] };
  const план = planRefund(чек, { 0: 1 });
  const возв = buildRefund({ receipt: чек, plan: план, reason: 'Брак', way: 'cash',
    approval: { ok: true },
    state: { lastNumber: 5, cashInDrawer: 20000, employee: { id: 'e1', name: 'А' },
      shift: { id: 's1' } },
    newId: () => 'v1' });

  ok(возв.ofReceiptId === 'r1', 'Возврат помнит свой чек');

  const s = чекДляСервера(возв);
  ok(s.refundOf === 'r1',
     '★ Ссылка ДОШЛА до сервера: иначе возврат ляжет как продажа и завысит выручку');
  ok(s.total === 220, `Сумма возврата верна: ${s.total}`);
}
