...
}
{
  ...
  ok(cart[0].name === 'Хлеб' && cart[0].qty === 2,
     'Второй хлеб сложился с первым: три «Хлеб ×1» выглядят как обсчёт');
  ok(cart[1].name === 'Молоко',
     'Порядок не сбился: молоко там же, где было');
}
{
  const cart = [];
...
// ── МАРКИ СНИМАЮТСЯ ВМЕСТЕ С ТОВАРОМ ───────────────────────────────
{
  ...
  ok(cart[0].codes.length === 1 && r.marksDropped === 2,
     '★ Убрали две бутылки — снялись две марки: свёртка внутри, забыть нельзя');
}
...
// ── ОТЛОЖЕННЫЕ: МОСТ ОТДАЁТ ОБЁРТКУ ───────────────────────────────
await (async () => {
  /* НАЙДЕНО ВЛАДЕЛЬЦЕМ: «отложенные не стали работать».
   *
   * Мост кассы отвечает { ok, data }, а свёртка читала данные
   * напрямую. Откладывание клало чек В ПУСТОТУ, а «Отложенные»
   * отвечали «Этот чек уже забрали» на живой чек. */
  let диск = {};
  const мост = {
    getState: async () => ({ ok: true, data: диск }),
    saveState: async (p) => { диск = { ...диск, ...p }; return { ok: true, data: диск }; },
  };

  const чек = [];
  let n = 0;
  addToCart(чек, { id: 'g1', name: 'Хлеб', price: 250 }, 1, () => 'l' + (++n));
  addToCart(чек, { id: 'g2', name: 'Молоко', price: 480 }, 1, () => 'l' + (++n));

  const п = await parkCart(мост, чек, { newId: () => 'p1', who: 'Айгуль' });
  ok(п.ok && (диск.parked || []).length === 1,
     '★ Через мост чек ЛОЖИТСЯ на диск, а не в пустоту');

  const назад = await unparkCart(мост, 'p1');
  ok(назад.ok && назад.lines.length === 2,
     '★ И возвращается целиком: две позиции');
  ok((диск.parked || []).length === 0,
     'Забранный чек убран из отложенных');

  const дважды = await unparkCart(мост, 'p1');
  ok(!дважды.ok && /уже забрали/.test(дважды.said),
     'Второй раз тот же чек не вернуть');

  // И простой склад тоже понимаем — на нём стоят остальные проверки.
  let прямой = {};
  const простой = {
    getState: async () => прямой,
    saveState: async (p) => { прямой = { ...прямой, ...p }; },
  };
  const п2 = await parkCart(простой, чек, { newId: () => 'p2' });
  ok(п2.ok && (прямой.parked || []).length === 1,
